...
TOOLS_SYSTEM_PROMPT = f"""You are a smart travel agency. Use the tools to look up information.
    ...
    CUSTOM PREFERENCES:
    - The user is a "Cheapest Traveler". ALWAYS prioritize the lowest prices for flights and accommodation.
    - For accommodation, look for Airbnbs or Hotels with "2 sharing max" (no hostels).
    - For Airbnbs/apartments, ensure there is a grocery store or supermarket nearby for cooking (use search_tool).
    - If there are "night trains" available that save on hotel costs, suggest them.
    - If "ice breaker cruise" or other activities are requested, use search_tool.
    - Provide "exact market studies" where possible (e.g., comparing a few options).
    - Always strictly follow the user's budget and sharing preferences.
...
TOOLS = [flights_finder, hotels_finder, search_tool]
...
class Agent:
    ...
    def email_sender(self, state: AgentState):
        ...
        return {'messages': [HumanMessage(content=email_response.content, name="html_itinerary")]}
